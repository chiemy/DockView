...
public class PianoMenuView extends LinearLayout {
    ...
    public PianoMenuView(Context context) {
        this(context, null);
    }

    public PianoMenuView(Context context, @MenuOrientation int orientation) {
        super(context);
        this.orientation = orientation;
        init();
    }

    public PianoMenuView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PianoMenuView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        if (attrs != null) {
            TypedArray t = context.obtainStyledAttributes(attrs, R.styleable.PianoMenuView);
            orientation = t.getInt(R.styleable.PianoMenuView_orientation, BOTTOM_TO_TOP);
            int layoutId = t.getResourceId(R.styleable.PianoMenuView_content_layout, 0);
            if (layoutId != 0) {
    ...
    private void init() {
        pianoView = new PianoView(getContext());
        switch (orientation) {
            case LEFT_TO_RIGHT:
